...
all_labels = list(pd.concat([df["NATURE"],df["INTITULE_DONNEES"],df["ACQUISITION_AUTOMATIQUE"],df["STOCKAGE_BRUT"],df["ACCES_BRUTES"],df["TRAITEMENT_DONNEES_BRUTES"],df["MOYEN_CALCUL"]]).unique())
...
colors_for_nodes = ["yellow"] * len(df['NATURE'].unique()) + ["white"] * (len(df["INTITULE_DONNEES"].unique())) + ["white"] * (len(df["ACQUISITION_AUTOMATIQUE"].unique())) + ["white"] * (len(df["STOCKAGE_BRUT"].unique())) + ["white"] * (len(df["ACCES_BRUTES"].unique())) + ["white"] * (len(df["TRAITEMENT_DONNEES_BRUTES"].unique())) + ["white"] * (len(df["MOYEN_CALCUL"].unique())) 
...
fig.update_layout(
    hovermode = 'x',
    title=dict(text="<b> ACQUISITIONS DES DONNEES GREENSCALE </b>", font=dict(color="black",size=18), x=0.3, y=0.01),
...
fig.add_annotation(dict(font=dict(color="black",size=18), x=0.27, y=1.1, showarrow=False, text='<b> ACQUISITION AUTO </b>'))
fig.add_annotation(dict(font=dict(color="black",size=18), x=0.5, y=1.1, showarrow=False, text='<b> STOCKAGE </b>'))
fig.add_annotation(dict(font=dict(color="black",size=18), x=0.7, y=1.1, showarrow=False, text='<b> ACCES </b>'))
fig.add_annotation(dict(font=dict(color="black",size=18), x=0.87, y=1.1, showarrow=False, text='<b> TRAITEMENT </b>'))
fig.add_annotation(dict(font=dict(color="black",size=18), x=1.05, y=1.1, showarrow=False, text='<b> MOYENS DE CALCUL </b>'))
