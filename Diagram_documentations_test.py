...
all_labels = list(pd.concat([df["NATURE"],df["INTITULE_DONNEES"],df["SOURCE"], df["REFERENCES"],df["MOYENS_PRODUCTION"],df["DOCS_ASSOCIES"],df["THESAURUS"],df["THESAURUS_OUI"]]).unique())
...
colors_for_nodes = ["yellow"] * len(df['NATURE'].unique()) + ["white"] * (len(df["INTITULE_DONNEES"].unique())) + ["white"] * (len(df["SOURCE"].unique())) + ["white"] * (len(df["REFERENCES"].unique())) + ["white"] * (len(df["MOYENS_PRODUCTION"].unique())) + ["white"] * (len(df["DOCS_ASSOCIES"].unique())) + ["white"] * (len(df["THESAURUS"].unique())) + ["white"] * (len(df["THESAURUS_OUI"].unique()))
...
fig.update_layout(
    hovermode = 'x',
    title=dict(text="<b> DOCUMENTATIONS DES DONNEES GREENSCALE </b>", font=dict(color="black",size=18), x=0.3, y=0.01),
...
fig.add_annotation(dict(font=dict(color="black",size=18), x=0.27, y=1.1, showarrow=False, text='<b> SOURCE </b>'))
fig.add_annotation(dict(font=dict(color="black",size=18), x=0.45, y=1.1, showarrow=False, text='<b> REFERENCES </b>'))
fig.add_annotation(dict(font=dict(color="black",size=18), x=0.57, y=1.1, showarrow=False, text='<b> MOYENS </b>'))
fig.add_annotation(dict(font=dict(color="black",size=18), x=0.8, y=1.1, showarrow=False, text='<b> DOCS ASSOCIES </b>'))
fig.add_annotation(dict(font=dict(color="black",size=18), x=0.95, y=1.1, showarrow=False, text='<b> THESAURUS </b>'))
